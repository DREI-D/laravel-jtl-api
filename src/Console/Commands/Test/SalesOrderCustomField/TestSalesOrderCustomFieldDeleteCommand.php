...
class TestSalesOrderCustomFieldDeleteCommand extends Command
{
    protected $signature = 'jtl-api:test:sales-order-custom-field-delete';

    protected $description = 'Tests the item custom field update endpoint';
    ...
    public function handle(): void
    {
        ...
        throw_if(count($response->customFieldValues) === 0);

        /** @var SalesOrderCustomFieldValueDto $customField */
        $customField = $response->customFieldValues[0];

        $response = app(SalesOrderCustomFieldRepository::class)->deleteSalesOrderCustomField(
            new DeleteSalesOrderCustomFieldRequest(
                salesOrderId: $salesOrder->id,
